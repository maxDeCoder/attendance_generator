...
print("Welcome to Attendance generation utility tool v1.0 \nby VEDANT JUMLE, FE-CE, DIV D, ROLL NO. 10")
...
threshold = int(
    input("how much time in minutes are students allowed to by outside of the meeting(enter 10 mins as just 10)?\n"))
...
ending_timestamp = maketimestamp(
    str(attendence_df["Timestamp"].values[0]).split(",")[0] + f", {ending_hour}:{ending_minutes}:00")
...
for name in names_list:
    ...
    information[name]["time"] = str(calculated_time)[:4] + "hrs"
    information[name]["status"] = "P" if determineStatus(calculated_time, starting_time, ending_time,
                                                         threshold) else "AB"
